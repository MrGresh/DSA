...
class Solution {
    public int maxDiffSubArrays(int[] arr) {
        ...
        // Left-to-right arrays to store max and min subarray sums ending at or before index i
        int[] leftMax = new int[n];
        int[] leftMin = new int[n];
        
        // Right-to-left arrays to store max and min subarray sums starting at or after index i
        ...
        // 3. Find the maximum absolute difference by splitting between i and i+1
        int maxDiff = 0;
        for (int i = 0; i < n - 1; i++) {
