...
public class Solution {
    // Recursive function with memoization to calculate the maximum points
    public static int f(int day, int last, int[][] points, int[][] dp) {
        ...
        if (day == 0) {
            int maxi = 0;
            // Calculate maximum points for day 0 by choosing an activity different from 'last'
            for (int i = 0; i <= 2; i++) {
                if (i != last) {
        ...
        for (int i = 0; i <= 2; i++) {
            if (i != last) {
                // Points for current activity + recursive call for the previous day
    ...
    public static int ninjaTraining(int n, int[][] points) {
        ...
        // Using 4 as the second dimension to handle the initial state (last = 3)
        int[][] dp = new int[n][4];
        for (int[] row : dp) {
            Arrays.fill(row, -1);
        }

        // Start from the last day (n-1) with '3' as the initial 'last' activity
